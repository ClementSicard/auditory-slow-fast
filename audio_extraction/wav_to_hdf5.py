...
def load_audio(root, file, target_sampling_rate):
    samples, sampling_rate = librosa.core.load(os.path.join(root, file), sr=None, mono=False)
    assert sampling_rate == target_sampling_rate, "Sampling rate of audio files should be {} ({})".format(
        target_sampling_rate, file
    )
