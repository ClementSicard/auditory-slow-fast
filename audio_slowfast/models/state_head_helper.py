...
class ResNetBasicHead(nn.Module):
    ...
    def __init__(
        ...
        if isinstance(num_classes, (list, tuple)):
            ...
            self.state_act = nn.Softmax(dim=1)  # Softmax on [B, 3, P]
    ...
    def project_state(self, x: torch.Tensor) -> torch.Tensor:
        ...
        """
        x_min_1 = self.projection_min_1(x)  # (B, P)
        x_0 = self.projection_0(x)  # (B, P)
        x_1 = self.projection_1(x)  # (B, P)

        # Concatenate them into a single tensor of size (B, 3, P)
        x = torch.cat([x_min_1, x_0, x_1], dim=1)
