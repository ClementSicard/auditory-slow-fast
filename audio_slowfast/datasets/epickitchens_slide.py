...
class EpicKitchensSlide(torch.utils.data.Dataset):
    def __init__(self, cfg, mode):
        assert mode in [
            ...
            "test",
            "train+val",
        ], "Split '{}' not supported for {}".format(mode, self.__class__)
        self.cfg = cfg
        self.mode = mode
        self.target_fps = 60
    ...
    def _construct_loader(self):
            ...
            for tup in pd.read_pickle(file).iterrows():
                if not self.cfg.TEST.SLIDE.ENABLE:
                    ...
                    if self.cfg.TEST.SLIDE.INSIDE_ACTION_BOUNDS:
                        ...
                        win_stop_sec = (
                            win_label_sec + (1 - self.cfg.TEST.SLIDE.LABEL_FRAME) * self.cfg.TEST.SLIDE.WIN_SIZE
                        )
        ...
        if self.cfg.TEST.SLIDE.ENABLE and not self.cfg.TEST.SLIDE.PER_ACTION_INSTANCE:
            for iii in range(len(self._video_records)):
                self._video_records[iii]._series["noun_class"] = np.array(
                    self._video_records[iii]._series["noun_class"]
                )
                self._video_records[iii]._series["verb_class"] = np.array(
                    self._video_records[iii]._series["verb_class"]
                )
            ...
            for i, frames in enumerate([frames_rgb, frames_flow]):
                ...
                frames = [
                    cv2.resize(
                        ...
                        (0, 0),
                        fx=scale,
                        fy=scale,  # The input order for OpenCV is w, h.
                    )
